...
def main():
    ...
    c = db.cursor()
    c.execute("select * from m_articles ;")
    art = c.fetchall()
    ...
    answers += br
    answers += br
    c.execute("select * from m_authors ;")
    aut = c.fetchall()
    ...
    c.execute \
        ("select date , cast(errors as decimal)/cast(total_requests as decimal) *"
         " 100 as per from dates_errors  order by per desc ;")
    er = c.fetchall()
